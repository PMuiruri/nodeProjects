...
const fatalError= err =>
  new Error(`Sorry! Error in our application ${err.message}`);

const checkPasswordSql = 'select role from User where username=? and userpassword=?';

const isAllowedSql = 'select username from User where username=(select username '+
...
module.exports = class SessionStorage{
	...
	hasSessionId(sessionId) {
		return new Promise(async (resolve, reject) => {
			try {
				let result = await this.sessionDb.doQuery(hasSessionIdSql,
					sessionId);
					resolve(result.length>0);
				} catch (err) {
					reject(fatalError(err));
...
addSession(sessionId, username) {
	return new Promise(async (resolve, reject) => {
		try {
			let result = await this.sessionDb.doQuery(insertSessionSql,
				sessionId,username);
